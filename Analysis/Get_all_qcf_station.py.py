...
def select_stations(station_code   = None, 
                    area           = [-90,-180,-60,180], 
                    metadata_fname = "ghcnm.tavg.v4.0.1.20220112.qcf.inv",
                    data_fname     = "ghcnm.tavg.v4.0.1.20220112.qcf.dat",
                    output_list    = 'ghcnm_selected_stations.csv',
                    output_path    =  "/data_qcf"):
...
if __name__ == "__main__":  
    ...
    parser.add_argument("-s","--stations_list", metavar='\b', type=str, help="Name of the file containing the list selected stations (default = 'stations.csv')", default = 'stations_qcf.csv')
    ...
    if missing_args == False:
        ...
        if os.path.exists(os.path.normpath(args.output_path) +  '/stations/') == False: os.mkdir(os.path.normpath(args.output_path) +  '/data_qcf/')
